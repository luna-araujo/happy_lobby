...
local function exportLayers(sprite, root_layer, filename, group_sep, data)
    for _, layer in ipairs(root_layer.layers) do
        ...
        if layer.isGroup then
            ...
            -- Create a folder for this layer and export each cel (frame) as its own file.
            local layer_folder = Dirname(filename) .. Sep .. layer.name
            os.execute("mkdir \"" .. layer_folder .. "\"")

            -- Helper to produce output filename for a cel/frame
            local function output_filename_for_cel(base_pattern, layer_folder, cel_index)
                local base = Basename(base_pattern)
                base = RemoveExtension(base)
                -- If user included {frame} token in pattern, replace it, otherwise append _frameN
                if string.find(base_pattern, "{frame}") then
                    local fpattern = base_pattern:gsub("{frame}", tostring(cel_index))
                    return layer_folder .. Sep .. Basename(fpattern)
                else
                    return layer_folder .. Sep .. base .. "_" .. tostring(cel_index) .. "." .. data.format
                end
            end
            ...
            if data.spritesheet then
                ...
                -- Export each cel/frame as its own full-canvas file (no trimming)
                local exported_count = 0
                for _, cel in ipairs(layer.cels) do
                    if cel.image then
                        local cel_index = (cel.frame and (cel.frame.frameNumber or cel.frame)) or exported_count + 1
                        local outname = output_filename_for_cel(filename, layer_folder, cel_index)
                        ...
                        app.command.SaveFile{ui=false, filename=outname}
                        app.command.CloseFile()
                        app.activeSprite = layer.sprite
                        sprite.selection = Selection()

                        exported_count = exported_count + 1
                    end
                end
                n_layers = n_layers + exported_count
            end
